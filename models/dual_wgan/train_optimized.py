#!/usr/bin/env python
"""Train dual-head WGAN with GPU optimizations for faster training."""
...
def main():
    ...
    # OPTIMIZATIONS:
    batch_size = 256  # Increased from 32 to 256 (8x larger)
    ...
    output_dir = Path(f'../../results/dual_wgan_optimized_{timestamp}')
    ...
    # OPTIMIZATION: Multi-worker data loading + pin_memory for faster GPU transfer
    ...
    # OPTIMIZATION: Mixed precision training scaler for ~2x speedup
    scaler = GradScaler()
    ...
    print(f"  Batch size: {batch_size} (8x larger)")
    ...
    for epoch in range(n_epochs):
        ...
        for i, real_data in enumerate(dataloader):
            ...
            for _ in range(n_critic):
                netC.zero_grad(set_to_none=True)  # More efficient than zero_grad()

                # Use mixed precision for critic
                with autocast():
        ...
        print(f"Epoch {epoch+1} completed in {epoch_time:.2f}s (avg: {avg_epoch_time:.2f}s, ETA: {eta_minutes:.1f} min)")
        ...
        if (epoch + 1) % 10 == 0:
            with torch.no_grad():
                ...
                plt.suptitle(f'Generated Profiles (Optimized) - Epoch {epoch+1}', fontsize=14, fontweight='bold')
                plt.tight_layout()
