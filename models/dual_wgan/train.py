...
class ProfileDataset(Dataset):
    def __init__(self, data_path, norm_params_path):
        ...
        labels_path = str(data_path).replace('X_raw.npy', 'y_labels.npy')
    ...
    def __getitem__(self, idx):
        return torch.FloatTensor(self.normalized_data[idx]), torch.tensor(self.labels[idx], dtype=torch.long)
    ...
    for epoch in range(n_epochs):
        for i, (real_data, real_labels) in enumerate(dataloader):
            ...
            for _ in range(n_critic):
                ...
                fake_data, _, _ = netG(noise, fake_labels if n_classes > 1 else None)

                critic_real = netC(real_data, real_labels if n_classes > 1 else None)
                critic_fake = netC(fake_data.detach(), fake_labels if n_classes > 1 else None)

                gp = compute_gradient_penalty(
                    netC, real_data, fake_data.detach(), device,
            ...
            critic_fake = netC(fake_data, fake_labels if n_classes > 1 else None)
        ...
        if (epoch + 1) % 10 == 0:
            with torch.no_grad():
                n_vis = 16
                vis_labels = torch.arange(n_vis, device=device) % max(n_classes, 1)
                noise = torch.randn(n_vis, nz, device=device)
                fake_data, fake_sigma, fake_mu = netG(noise, vis_labels if n_classes > 1 else None)
                ...
                for idx in range(n_vis):
                    ...
                    if n_classes > 1:
                        ax.set_title(f'Class {vis_labels_np[idx]}', fontsize=7)
