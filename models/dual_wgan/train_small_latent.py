#!/usr/bin/env python
"""
Train dual-head WGAN with smaller latent space (nz=32) for comparison.

This experiment tests whether a smaller latent dimension is sufficient
for generating correlated sigma and mu profiles with K=51 layers.
"""
...
def denormalize_profiles(data, norm_params, K):
    ...
    sigma = (sigma_normalized + 1) / 2 * (norm_params['sigma_max'] - norm_params['sigma_min']) + norm_params['sigma_min']
    mu = (mu_normalized + 1) / 2 * (norm_params['mu_max'] - norm_params['mu_min']) + norm_params['mu_min']
...
def main():
    ...
    output_dir = Path(f'./results/dual_wgan_nz{nz}_{timestamp}')
    ...
    config = {
        ...
        'n_critic': n_critic,
        'lambda_gp': lambda_gp,
        'experiment': f'smaller_latent_nz{nz}',
    }
    with open(output_dir / 'config.json', 'w') as f:
        json.dump(config, f, indent=2)

    print(f"\n{'='*60}")
    print(f"Dual WGAN - Small Latent Space Experiment (nz={nz})")
    print(f"{'='*60}")
    print(f"  K (layers): {K}")
    print(f"  nz (latent): {nz}")
    print(f"  nz/(2K) ratio: {nz/(2*K):.2f}")
    ...
    for epoch in range(n_epochs):
        ...
        if (epoch + 1) % 10 == 0:
            with torch.no_grad():
                ...
                plt.suptitle(f'Generated Profiles (nz={nz}) - Epoch {epoch+1}', fontsize=14, fontweight='bold')
                plt.tight_layout()
    ...
    plt.suptitle(f'Training Curves (nz={nz})', fontsize=14, fontweight='bold')
    plt.tight_layout()
